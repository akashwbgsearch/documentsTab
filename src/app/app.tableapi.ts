...
export class TableComponent {
	...
	constructor(private element: ElementRef, private apiConnectionService:APIConnectionService,private excelService: ExcelService){
	...
	callAPI(rowsCount,checkVal){	
		...
		this.apiConnectionService.getAPIResponse(this.apiUrl).subscribe((response)=> {	
	...
	downloadExcel(){
		...
		this.apiConnectionService.getAPIResponse(this.inputApiUrl+'&rows=500').subscribe((response)=> {	
		...
		if(response.total>'0'){  
			...
			if(this.tabType=='projects'){
				...
				this.excelJsonArrayResponse	=excelJsonArrayResponse;
				this.excelService.exportProjectsExcelFile(this.excelJsonArrayResponse, this.excelName, this.tableHeaders); 				
			}	
			...
			if(this.tabType=='archival'){
				...
				this.excelJsonArrayResponse	=excelJsonArrayResponse;
				this.excelService.exportArchivalExcelFile(this.excelJsonArrayResponse, this.excelName, this.tableHeaders); 	
			}
		}
		});
